...
- **指令列表定位**：在会话页右侧提供类似 DeepSeek 网页对话的**指令列表**面板，一键列出会话中的全部用户指令，点击即可平滑滚动定位到对应历史消息，不用再一点一点往上翻；
...
- `lib/client.js`：浏览器半边，注册三个插槽：
  ...
  2. `conversation.input.dock`：输入框上方的历史穿梭提示条（v0.2.0 新增）；
  3. `shell.overlay`：右侧浮动面板本体。
...
- 历史穿梭收录范围：`collectHistory` 中放宽 `kind` 判断（目前与列表一致，只收 `user` / `steering`）
