window.__ModuleLoader__.load({
	...
	factory: (require) => {
				...
				React.createElement(
					...
					items.map((item, i) =>
						React.createElement("button", {
							...
							style: { top: padY + i * (BAR_H + BAR_GAP), width: Math.round(barWidth(i)), height: BAR_H },
							title: item.text,
		...
		const CSS = `
.dsh-rail{position:fixed;box-sizing:border-box;z-index:50;pointer-events:auto;border-radius:12px;background:color-mix(in srgb,var(--dsw-alias-bg-layer-2) 55%,transparent);box-shadow:0 4px 16px color-mix(in srgb,var(--dsw-alias-label-primary) 10%,transparent);animation:dsh-rail-in .22s ease-out}
...
.dsh-rail-bar{position:absolute;left:6px;height:4px;border:none;border-radius:3px;padding:0;cursor:pointer;background:color-mix(in srgb,var(--dsw-alias-label-primary) 24%,transparent);transition:width .18s ease,background-color .15s ease}
