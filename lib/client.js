window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function Rail({ useSessions }) {
			...
			// 全量历史：来自宿主 RPC（含未加载的更早指令）
			React.useEffect(() => {
				if (current === undefined) { setFull(null); return; }
				let alive = true;
				setFull(null);
				Promise.resolve(host.call("msgjump.fullHistory", { sessionId: current }))
					.then((res) => { if (alive && res && Array.isArray(res.items)) setFull(res.items); })
					.catch(() => { /* 宿主不可用时回退到已加载窗口 */ });
				return () => { alive = false; };
			}, [current]);
