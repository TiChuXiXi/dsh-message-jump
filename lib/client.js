window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function Rail({ useSessions }) {
			...
			// 每个刻度占一格（条高+间距）整格可命中：视觉上保留条间间隙，
			// 但鼠标落在间隙上也会命中该格对应的刻度，不会"点空"。
				...
				hovered &&
					React.createElement(
						"div",
						{
							...
							style: {
								top: Math.max(geom.top + 8, Math.min(geom.bottom - 98, railTop + padY + hoverIdx * slotH + slotH / 2 - 45)),
							},
